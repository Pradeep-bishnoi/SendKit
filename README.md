...
```bash
npm i @pradeepbishnoiorg/sendkit   # changes to do here
...
```bash
npm i @pradeepbishnoiorg/sendkit  # update this things
```
...
```bash
npm i @pradeepbishnoiorg/sendkit
```
...
```json
{
  "mcpServers": {
    "sendkit": {
      "command": "npx",
      "args": ["-y", "@pradeepbishnoiorg/sendkit-mcp"],
      "environment": {
        "TELEGRAM_BOT_TOKEN": "<bot-token>"
      }
    }
...
```bash
https://github.com/Pradeep-bishnoi/SendKit/tree/main/skills/sendkit
```

The Skill tells agents when to use the MCP `telegram` tool, when to fall back to the CLI, why `--json` matters for parsing, and why `@pradeepbishnoiorg/sendkit-core` is only an implementation detail.
